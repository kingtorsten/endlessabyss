...
public class MutantSquid extends WaterMobEntity {
...
protected void registerGoals() {
   this.goalSelector.addGoal(0, new MutantSquid.MoveRandomGoal(this));
   this.goalSelector.addGoal(1, new MutantSquid.FleeGoal());
}
...
public static boolean func_223365_b(EntityType<MutantSquid> p_223365_0_, IWorld p_223365_1_, SpawnReason p_223365_2_, BlockPos p_223365_3_, Random p_223365_4_) {
...
class FleeGoal extends Goal {
   ...
   public boolean shouldExecute() {
      LivingEntity livingentity = MutantSquid.this.getRevengeTarget();
      if (MutantSquid.this.isInWater() && livingentity != null) {
         return MutantSquid.this.getDistanceSq(livingentity) < 100.0D;
      } else {
         return false;
      }
   }
   ...
   public void tick() {
      ...
      if (livingentity != null) {
         Vec3d vec3d = new Vec3d(MutantSquid.this.posX - livingentity.posX, MutantSquid.this.posY - livingentity.posY, MutantSquid.this.posZ - livingentity.posZ);
         BlockState blockstate = MutantSquid.this.world.getBlockState(new BlockPos(MutantSquid.this.posX + vec3d.x, MutantSquid.this.posY + vec3d.y, MutantSquid.this.posZ + vec3d.z));
         IFluidState ifluidstate = MutantSquid.this.world.getFluidState(new BlockPos(MutantSquid.this.posX + vec3d.x, MutantSquid.this.posY + vec3d.y, MutantSquid.this.posZ + vec3d.z));
         if (ifluidstate.isTagged(FluidTags.WATER) || blockstate.isAir()) {
            ...
            MutantSquid.this.setMovementVector((float)vec3d.x / 20.0F, (float)vec3d.y / 20.0F, (float)vec3d.z / 20.0F);
         }

         if (this.tickCounter % 10 == 5) {
            MutantSquid.this.world.addParticle(ParticleTypes.BUBBLE, MutantSquid.this.posX, MutantSquid.this.posY, MutantSquid.this.posZ, 0.0D, 0.0D, 0.0D);
         }
...
class MoveRandomGoal extends Goal {
   private final MutantSquid mutantSquid;

   public MoveRandomGoal(MutantSquid p_i48823_2_) {
      this.mutantSquid = p_i48823_2_;
   }
    ...
    */
   public void tick() {
      int i = this.mutantSquid.getIdleTime();
      if (i > 100) {
         ((MutantSquid) this.mutantSquid).setMovementVector(0.0F, 0.0F, 0.0F);
      } else if (this.mutantSquid.getRNG().nextInt(50) == 0 || !this.mutantSquid.inWater || !((MutantSquid) this.mutantSquid).hasMovementVector()) {
         float f = ((LivingEntity) this.mutantSquid).getRNG().nextFloat() * ((float)Math.PI * 2F);
         float f1 = MathHelper.cos(f) * 0.2F;
         float f2 = -0.1F + ((LivingEntity) this.mutantSquid).getRNG().nextFloat() * 0.2F;
         float f3 = MathHelper.sin(f) * 0.2F;
         ((MutantSquid) this.mutantSquid).setMovementVector(f1, f2, f3);
      }
